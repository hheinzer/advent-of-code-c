...
typedef enum Mineral { ORE, CLY, OBS, GEO, N } Mineral;
...
long _dfs(Blueprint *bp, long time, Miningstate s, Dict *cache, long *best)
{
    ...
    if (time == 0) { return s.amt[GEO]; }

    // check cache
    char key[256] = "";
    Item *item = dict_find(cache,
        KEY(key, "%ld %ld %ld %ld %ld %ld %ld %ld %ld ", time, s.bot[ORE], s.bot[CLY],
            s.bot[OBS], s.bot[GEO], s.amt[ORE], s.amt[CLY], s.amt[OBS], s.amt[GEO]));
    if (item) { return *(long *)item->data; }

    // check if this state has the potential to beat the best state
    long pot = s.amt[GEO];
    for (long i = 0; i < time; ++i) { pot += s.bot[GEO] + i; }
    if (pot < *best) { return s.amt[GEO]; }

    // compute state
    long max_geo = s.amt[GEO] + s.bot[GEO] * time;
    for (size_t ibot = 0; ibot < N; ++ibot) {
        // check if we need this robot
        if ((ibot != GEO) && (s.bot[ibot] >= bp->max[ibot])) { continue; }
        ...
        if (can_wait) {
            ...
            // mine minerals
            for (size_t j = 0; j < N; ++j) { ns.amt[j] += ns.bot[j] * (wait + 1); }

            // build robot
            for (size_t j = 0; j < N; ++j) { ns.amt[j] -= bp->amt[ibot][j]; }
            ns.bot[ibot] += 1;
...
int main(void)
{
    ...
    for (size_t i = 0; i < n_lines; ++i) {
        sscanf(line[i],
            ...
            &bp[i].amt[ORE][ORE], &bp[i].amt[CLY][ORE], &bp[i].amt[OBS][ORE],
            &bp[i].amt[OBS][CLY], &bp[i].amt[GEO][ORE], &bp[i].amt[GEO][OBS]);
        for (size_t j = 0; j < N - 1; ++j) {
    ...
    // part 2
    long prod_geo_cnt = 1;
    for (size_t i = 0; i < MIN(n_lines, 3); ++i) { prod_geo_cnt *= dfs(&bp[i], 32); }
    printf("%ld\n", prod_geo_cnt);
